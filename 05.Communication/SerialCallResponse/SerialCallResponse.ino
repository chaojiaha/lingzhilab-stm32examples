/*
串口交互，当没有发送数据时一直发送’A‘， 当有数据时发送三次传感器的数据
*/

int firstSensor = 0;    // 三个传感器相应的数据
int secondSensor = 0;  
int thirdSensor = 0;    
int inByte = 0;         //串口获取的数据

void setup() {
    Serial.begin(9600); 
    pinMode(0, INPUT_ANALOG); // 0脚接模拟输入
    pinMode(1, INPUT_ANALOG); // 1脚接模拟输入
    pinMode(2, INPUT);        // 2脚接数字输入
    establishContact();       // 串口没有数据时发送字符'A'
}
